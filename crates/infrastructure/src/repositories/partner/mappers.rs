...
pub fn row_to_partner(row: PartnerRow) -> Result<Partner, AppError> {
    let sharing_type = match row.profit_sharing_type.as_str() {
        ...
        other => {
            eprintln!("⚠️ Unknown profit_sharing_type '{}' for partner {}, defaulting to Manual", other, row.id);
    ...
    let exchange_rate = Decimal::from_str(&row.exchange_rate)
        .map_err(|e| {
            eprintln!("⚠️ Corrupt exchange_rate '{}' for partner {}: {}", row.exchange_rate, row.id, e);
            AppError::Infrastructure(format!("سعر صرف غير صالح للشريك {}: {}", row.name, e))
        })?;

    let amount_local = Decimal::from_str(&row.amount_local)
        .map_err(|e| {
            eprintln!("⚠️ Corrupt amount_local '{}' for partner {}: {}", row.amount_local, row.id, e);
            AppError::Infrastructure(format!("مبلغ محلي غير صالح للشريك {}: {}", row.name, e))
        })?;

    let amount_original = Decimal::from_str(&row.amount_original)
        .map_err(|e| {
            eprintln!("⚠️ Corrupt amount_original '{}' for partner {}: {}", row.amount_original, row.id, e);
    ...
    let linked_account_id = match &row.linked_account_id {
        Some(id) => Some(AccountId::from_str(id).map_err(|e| {
            eprintln!("⚠️ Corrupt linked_account_id '{}' for partner {}: {}", id, row.id, e);
            AppError::Infrastructure(format!("معرف حساب رأسمال غير صالح للشريك {}: {}", row.name, e))
        })?),
        None => None,
    };
    ...
    let drawings_account_id = match &row.drawings_account_id {
        Some(id) => Some(AccountId::from_str(id).map_err(|e| {
            eprintln!("⚠️ Corrupt drawings_account_id '{}' for partner {}: {}", id, row.id, e);
            AppError::Infrastructure(format!("معرف حساب مسحوبات غير صالح للشريك {}: {}", row.name, e))
        })?),
        None => None,
    };
    ...
    let current_account_id = match &row.current_account_id {
        Some(id) => Some(AccountId::from_str(id).map_err(|e| {
            eprintln!("⚠️ Corrupt current_account_id '{}' for partner {}: {}", id, row.id, e);
            AppError::Infrastructure(format!("معرف حساب جاري غير صالح للشريك {}: {}", row.name, e))
        })?),
        None => None,
    };
    ...
    let created_at = DateTime::parse_from_rfc3339(&row.created_at)
        ...
        .map_err(|e| {
            eprintln!("⚠️ Corrupt created_at '{}' for partner {}: {}", row.created_at, row.id, e);
    ...
    let updated_at = DateTime::parse_from_rfc3339(&row.updated_at)
        .map(|d| d.with_timezone(&Utc))
        .map_err(|e| {
            eprintln!("⚠️ Corrupt updated_at '{}' for partner {}: {}", row.updated_at, row.id, e);

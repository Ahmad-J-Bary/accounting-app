...
pub fn row_to_payment(row: PaymentRow) -> Result<Payment, AppError> {
    ...
    Ok(Payment {
        ...
        customer_id: row.customer_id.and_then(|id| id.parse::<u64>().ok()).map(CustomerId::from_u64),
        supplier_id: row.supplier_id.and_then(|id| id.parse::<u64>().ok()).map(SupplierId::from_u64),

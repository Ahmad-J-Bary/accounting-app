...
pub async fn save(pool: &SqlitePool, lot: &InventoryLot) -> Result<(), AppError> {
    sqlx::query(
        "INSERT INTO inventory_lots (id, material_id, purchase_invoice_id, movement_id, quantity_original, quantity_remaining, unit_cost_base, raw_unit_cost_base, currency_code, fx_rate, purchase_date, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    )

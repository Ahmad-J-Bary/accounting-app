use super::models::{AssetCategoryRow, AssetMovementRow, AssetRow, DepreciationScheduleRow};
use application::errors::AppError;
use chrono::{DateTime, Utc};
use domain::assets::{
    AssetCategory, AssetMovement, AssetMovementType, AssetStatus, AssetType, DepreciationMethod,
    DepreciationSchedule, DepreciationStatus, FixedAsset, FixedAssetId,
};
...
pub fn row_to_asset(row: AssetRow) -> Result<FixedAsset, AppError> {
    Ok(FixedAsset {
        ...
        purchase_cost: Money::new(
            Decimal::from_str(&row.purchase_cost).unwrap_or_default(),
            currency_from_code(&row.currency),
        ),
        ...
        salvage_value: row
            ...
            .map(|d| Money::new(d, currency_from_code(&row.currency))),
        accumulated_depreciation: Money::new(
            Decimal::from_str(&row.accumulated_depreciation).unwrap_or_default(),
            currency_from_code(&row.currency),
        ),
        ...
        asset_account_id: Uuid::parse_str(&row.asset_account_id)
            .map_err(|e| AppError::Infrastructure(e.to_string()))?,
        depreciation_account_id: Uuid::parse_str(&row.depreciation_account_id)
            .map_err(|e| AppError::Infrastructure(e.to_string()))?,
        accumulated_depreciation_account_id: Uuid::parse_str(
...
pub fn row_to_movement(row: AssetMovementRow) -> AssetMovement {
    AssetMovement {
        ...
        date: DateTime::parse_from_rfc3339(&row.movement_date)
            .map(|d| d.with_timezone(&Utc))
            .unwrap_or_else(|_| Utc::now()),
        quantity: row.quantity.and_then(|s| Decimal::from_str(&s).ok()),
        amount: Money::new(
            Decimal::from_str(&row.amount).unwrap_or_default(),
            row.currency
                .as_deref()
                .map(currency_from_code)
                .unwrap_or_else(|| currency_from_code("")),
        ),
...
pub fn row_to_schedule(row: DepreciationScheduleRow) -> DepreciationSchedule {
    DepreciationSchedule {
        ...
        period_date: DateTime::parse_from_rfc3339(&row.period_date)
            .map(|d| d.with_timezone(&Utc))
            .unwrap_or_else(|_| Utc::now()),
        depreciation_amount: Money::new(
            Decimal::from_str(&row.depreciation_amount).unwrap_or_default(),
            row.currency
                .as_deref()
                .map(currency_from_code)
                .unwrap_or_else(|| currency_from_code("")),
        ...
        remaining_value: Money::new(
            Decimal::from_str(&row.remaining_value).unwrap_or_default(),
            row.currency
                .as_deref()
                .map(currency_from_code)
                .unwrap_or_else(|| currency_from_code("")),

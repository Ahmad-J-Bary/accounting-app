...
pub async fn list_categories(
    ...
    let rows = sqlx::query_as::<_, AssetCategoryRow>(
        "SELECT * FROM asset_categories WHERE asset_type = ?",
    )
...
pub async fn list_movements_by_asset(
    pool: &SqlitePool,
    asset_id: &Uuid,
) -> Result<Vec<AssetMovement>, AppError> {
    let rows = sqlx::query_as::<_, AssetMovementRow>(
        "SELECT * FROM asset_movements WHERE asset_id = ? ORDER BY movement_date DESC",
    )
...
pub async fn list_all_movements(pool: &SqlitePool) -> Result<Vec<AssetMovement>, AppError> {
    let rows = sqlx::query_as::<_, AssetMovementRow>(
        "SELECT * FROM asset_movements ORDER BY movement_date DESC",
    )
    .fetch_all(pool)
    .await
    .map_err(|e| AppError::Infrastructure(e.to_string()))?;
...
pub async fn get_depreciation_schedule(
    ...
    let rows = sqlx::query_as::<_, DepreciationScheduleRow>(
        "SELECT * FROM depreciation_schedules WHERE fixed_asset_id = ? ORDER BY period_date ASC",
    )
    .bind(asset_id.to_string())
    .fetch_all(pool)
    .await

...
pub fn row_to_purchase_return(
    row: PurchaseReturnRow,
    lines: Vec<PurchaseReturnLineRow>,
) -> Result<PurchaseReturn, AppError> {
    let id = PurchaseReturnId::from_str(&row.id)
        .map_err(|_| AppError::Invalid("معرف غير صالح".into()))?;
    let supplier_id = SupplierId::from_str(&row.supplier_id)
        .map_err(|_| AppError::Invalid("معرف مورد غير صالح".into()))?;

...
pub fn row_to_sales_return(
    row: SalesReturnRow,
    lines: Vec<SalesReturnLineRow>,
) -> Result<SalesReturn, AppError> {
    let id =
        SalesReturnId::from_str(&row.id).map_err(|_| AppError::Invalid("معرف غير صالح".into()))?;
    let customer_id = CustomerId::from_str(&row.customer_id)
        .map_err(|_| AppError::Invalid("معرف عميل غير صالح".into()))?;

...
pub fn row_to_movement(row: StockMovementRow) -> Result<StockMovement, AppError> {
    ...
    Ok(StockMovement {
        ...
        material_id: MaterialId(
            Uuid::parse_str(&row.material_id).map_err(|e| AppError::Invalid(e.to_string()))?,
        ),
        ...
        movement_date: DateTime::parse_from_rfc3339(&row.movement_date)

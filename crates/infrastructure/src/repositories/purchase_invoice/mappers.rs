...
use domain::shared::ids::{PurchaseInvoiceId, SupplierId, MaterialId, AccountId};
...
pub fn row_to_invoice(
    ...
    Ok(PurchaseInvoice {
        id: PurchaseInvoiceId(Uuid::parse_str(&row.id).map_err(|e| AppError::Infrastructure(e.to_string()))?),
        ...
        status,
        invoice_date: DateTime::parse_from_rfc3339(&row.invoice_date).map(|d| d.with_timezone(&Utc)).unwrap_or_else(|_| Utc::now()),
        due_date: row.due_date.as_ref().and_then(|d| DateTime::parse_from_rfc3339(d).map(|dt| dt.with_timezone(&Utc)).ok()),
...
pub fn item_row_to_item(row: PurchaseInvoiceItemRow) -> Result<PurchaseInvoiceItem, AppError> {
    Ok(PurchaseInvoiceItem {
        ...
        unit_id: row.unit_id,
        conversion_factor: row.conversion_factor.and_then(|s| Decimal::from_str(&s).ok()),
...
pub fn cost_row_to_cost(row: PurchaseInvoiceAdditionalCostRow) -> Result<PurchaseAdditionalCost, AppError> {
    Ok(PurchaseAdditionalCost {
        id: row.id,
        description: row.description,
        account_id: AccountId(Uuid::parse_str(&row.account_id).map_err(|e| AppError::Infrastructure(e.to_string()))?),

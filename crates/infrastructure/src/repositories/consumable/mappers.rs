...
pub fn row_to_consumable(row: ConsumableRow) -> Result<Consumable, AppError> {
    Ok(Consumable {
        id: ConsumableId(Uuid::parse_str(&row.id).map_err(|e| AppError::Infrastructure(e.to_string()))?),
        code: row.code,
        name: row.name,
        ...
        expense_account_id: Uuid::parse_str(&row.expense_account_id).map_err(|e| AppError::Infrastructure(e.to_string()))?,

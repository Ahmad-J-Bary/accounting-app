...
use domain::shared::ids::{MaterialCategoryId, MaterialId, MaterialUnitId};
...
pub fn row_to_material(
    ...
    Ok(Material {
        ...
        default_purchase_unit_id: row
            .default_purchase_unit_id
            .and_then(|id| Uuid::parse_str(&id).ok().map(MaterialUnitId)),
        default_sale_unit_id: row
...
pub fn row_to_purchase_price(
    row: MaterialPurchasePriceRow,
) -> Result<MaterialPurchasePrice, AppError> {
    Ok(MaterialPurchasePrice {
        id: row.id,
        unit_id: MaterialUnitId(
            Uuid::parse_str(&row.unit_id).map_err(|e| AppError::Infrastructure(e.to_string()))?,
        ),
...
pub fn row_to_unit(row: MaterialUnitRow) -> Result<MaterialUnit, AppError> {
    Ok(MaterialUnit {
        ...
        name: row.name,
        conversion_factor: Decimal::from_str(&row.conversion_factor)

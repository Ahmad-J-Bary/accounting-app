use super::mappers::{row_to_material, row_to_purchase_price, row_to_sale_price, row_to_unit};
use super::models::{MaterialPurchasePriceRow, MaterialRow, MaterialSalePriceRow, MaterialUnitRow};
...
const MATERIAL_FIELDS: &str = "id, name, name_en, barcode, code, minimum_stock, notes, image_path, default_purchase_unit_id, default_sale_unit_id, default_purchase_currency, default_sale_currency, default_warehouse_id, has_expiry, expiry_alert_before_days, created_at, updated_at";
...
pub async fn find_by_code_or_barcode(
    pool: &SqlitePool,
    code_or_barcode: &str,
...
pub async fn list_all(pool: &SqlitePool) -> Result<Vec<Material>, AppError> {
    ...
    for r in rows {
        ...
        let purchase_prices = get_purchase_prices(pool, &r.id).await?;
        let sale_prices = get_sale_prices(pool, &r.id).await?;
        materials.push(row_to_material(
            r,
            units,
            cat_ids,
            purchase_prices,
...
pub async fn get_purchase_prices(
    pool: &SqlitePool,
    material_id: &str,
...
pub async fn get_sale_prices(
    pool: &SqlitePool,
    material_id: &str,
) -> Result<Vec<MaterialSalePrice>, AppError> {
    let rows = sqlx::query_as::<_, MaterialSalePriceRow>(
        "SELECT id, material_id, unit_id, tier, price, price_base, min_price, min_price_base, max_quantity, max_quantity_unit_id, currency, updated_at FROM material_sale_prices WHERE material_id = ?"
    )
    .bind(material_id)
    .fetch_all(pool)
    .await
...
pub async fn get_category_ids(
    pool: &SqlitePool,
    material_id: &str,
) -> Result<Vec<MaterialCategoryId>, AppError> {
    ...
    for r in rows {
        ...
        ids.push(MaterialCategoryId(
            Uuid::parse_str(&category_id).map_err(|e| AppError::Infrastructure(e.to_string()))?,
        ));
    }
    Ok(ids)
}
...
pub async fn get_units(
    pool: &SqlitePool,
    material_id: &str,
) -> Result<Vec<MaterialUnit>, AppError> {

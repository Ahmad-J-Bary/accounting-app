...
fn row_to_product(row: ProductRow) -> Result<Product, AppError> {
    Ok(Product {
        ...
        name: row.name,
        code: row.code,
        unit_price: Money::syp(Decimal::from_str(&row.unit_price).unwrap_or(Decimal::ZERO)),
        cost_price: Money::syp(Decimal::from_str(&row.cost_price).unwrap_or(Decimal::ZERO)),

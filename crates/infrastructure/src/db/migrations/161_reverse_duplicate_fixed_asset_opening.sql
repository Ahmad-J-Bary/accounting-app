...
-- whenever the opening-preparation gate is not active. When the migration's
-- fixed-asset items derive the same accounts (wizard -> migration lines ->
-- posted aggregate), the SAME balance is booked twice in the GL:
--   once by the per-asset GeneralJournal, once by the migration aggregate.
--
-- Migration 158 only reversed per-entity Account/MaterialOpeningBalance
-- journals, so GeneralJournal fixed-asset openings kept double-counting.
-- This migration inserts true Reversal journals (debit/credit swapped, the
-- same shape as 158 / `create_reversal`) for every posted fixed-asset opening
-- journal whose balance is duplicated by a POSTED migration aggregate journal,
-- links each reversal via `reversal_of_entry_id`, and marks the originals
-- `Reversed` with `reversed_at`. Nothing is deleted: the audit trail survives
-- and the migration aggregate keeps the canonical GL position.
--
-- Idempotency: every reversal is guarded by the unique
-- `idx_journal_reversal_unique` and a recorded (source_type, source_id); a
-- fixed-asset opening already reversed at runtime (or by a prior run) is a
-- no-op, and the UPDATE only touches rows that carry a reversal here.
--
-- Criteria for a fixed-asset opening journal to be reversed:
--   * journal_type = 'GeneralJournal'
--   * description LIKE 'إضافة أصل سابق%'  (only the "existing" opening booking)
--   * status = 'Posted', reversed_at IS NULL
--   * source is NOT a migration artifact (not `opening_balance:`,
--     `residual_classification:`, `ob_reversal:`)
...
-- ---- Reverse the duplicated fixed-asset opening journals --------------------
...
SELECT
    '5f161000-0000-4000-8000-' || substr(dupe.id, 25)                     AS id,
...
FROM (
    SELECT DISTINCT je.id, je.entry_number, je.entry_date, je.description,
     ...
     WHERE je.status = 'Posted'
       ...
       AND je.journal_type = 'GeneralJournal'
       AND je.description LIKE 'إضافة أصل سابق%'
...
SELECT
    '90000000-0000-4000-8000-' || substr(orig_line.id, 25)                  AS id,
 ...
 WHERE id IN (
     SELECT r.reversal_of_entry_id
      ...
      WHERE r.journal_type = 'Reversal'
        ...
        AND r.source_id LIKE 'per_entity_opening_reversal:5f161000%'
 );
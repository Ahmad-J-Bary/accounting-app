...
async fn run_opening_lifecycle(pool: &Arc<sqlx::SqlitePool>) -> String {
    ...
    assert_eq!(
        draft.0.source_system.as_deref(),
        Some("Legacy"),
        "source provenance must persist"
    );
    ...
    assert_eq!(
        posted.debit_total, posted.credit_total,
        "posting journal must balance"
    );
...
async fn existing_company_full_lifecycle_reaches_locked_and_persists_journal() {
    ...
    assert_eq!(
        opening_journals, 1,
        "exactly one opening journal, sourced to the migration"
    );

    // Live ledger reflects the opening: cash +5000, equity −5000 (credit).
    assert!(
        close_enough(ledger_balance(&pool, &cash).await, 5000.0),
        "cash ledger must carry the opening"
    );
    assert!(
        close_enough(ledger_balance(&pool, &equity).await, -5000.0),
        "equity ledger must carry the opening (credit)"
    );
    ...
    assert_eq!(
        ...
        "no duplicate AccountOpeningBalance journal"
    );
...
async fn after_lock_company_behaves_like_new_for_capital_contribution() {
    ...
    assert!(
        close_enough(ledger_balance(&pool, &cash).await, 5750.0),
        "cash must increase by 750 after lock"
    );
    assert!(
        close_enough(ledger_balance(&pool, &cap_id).await, -750.0),
        "capital ledger must increase by 750 (credit)"
    );
}

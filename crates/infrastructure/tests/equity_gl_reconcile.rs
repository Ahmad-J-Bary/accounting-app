...
async fn sql_net_balance(pool: &sqlx::SqlitePool, account_id: &AccountId) -> Decimal {
    ...
    for (c, d) in rows {
        net +=
            Decimal::from_str(&c).unwrap_or_default() - Decimal::from_str(&d).unwrap_or_default();
    }
    net
}
...
async fn equity_statement_reconciles_with_partner_ledgers() {
    ...
    .await;
    let (b_id, b_capital, b_drawings, b_current) = seed_partner(
        &pool,
        "P2",
        "519912",
        "449912",
    ...
    let mut seed = JournalEntry::new(
        ...
        vec![
            JournalLine::new(
                obe.id,
                MonetaryAmount::from_base(Decimal::from(600), c.clone()),
    ...
    let row_a = dto
        .rows
        .iter()
        .find(|r| r.partner_id == a_id.to_string())
        .expect("row a");
    let row_b = dto
        .rows
        .iter()
        .find(|r| r.partner_id == b_id.to_string())
        .expect("row b");
    ...
    assert_eq!(
        Decimal::from_str(&row_b.total_equity).unwrap(),
        cap_b + cur_b + drw_b
    );
    ...
    assert_eq!(
        Decimal::from_str(&row_a.capital_registered).unwrap(),
        Decimal::from(1000)
    );
    assert_eq!(
        Decimal::from_str(&row_b.capital_registered).unwrap(),
        Decimal::from(3000)
    );
    ...
    assert_eq!(
        Decimal::from_str(&row_a.profit_allocated).unwrap() - loss_a,
        cur_a
    );
    assert_eq!(
        Decimal::from_str(&row_b.profit_allocated).unwrap() - loss_b,
        cur_b
    );
}
...
async fn equity_statement_date_range_filtering() {
    ...
    for (full_row, wide_row) in full.rows.iter().zip(wide.rows.iter()) {
        ...
        assert_eq!(
            full_row.total_equity, wide_row.total_equity,
            "wide range must match full for {}",
            full_row.partner_name
        );
    }
    ...
    assert_eq!(
        full.rows.len(),
        narrow.rows.len(),
        "same partner count even with narrow range"
    );
    // No entries in the narrow range, so period profit/drawings must be zero
    for row in &narrow.rows {
        assert_eq!(
            row.period_profit, "0",
            "no period profit for {}",
            row.partner_name
        );
        assert_eq!(
            row.period_drawings, "0",
            "no period drawings for {}",
            row.partner_name
        );
    }
}

...
use application::use_cases::fiscal_period::{
    CreateFiscalPeriodCommand, CreateFiscalPeriodUseCase,
};
...
async fn build_pool() -> Arc<sqlx::SqlitePool> {
    ...
    path.push(format!("acc_period_current_{}.sqlite", uuid::Uuid::new_v4()));
...
async fn transaction_date_resolves_to_the_containing_period_only() {
    ...
    let h1 = uc.execute(cmd("2026-01-01T00:00:00Z", "2026-06-30T23:59:59Z")).await.expect("create H1");
    let h2 = uc.execute(cmd("2026-07-01T00:00:00Z", "2026-12-31T23:59:59Z")).await.expect("create H2");

    // Inside H1 → resolves to H1 and only H1.
    let in_h1 = repo.find_by_date(utc("2026-03-15T10:00:00Z")).await.unwrap();
    assert_eq!(in_h1.len(), 1, "a March date resolves to exactly one period");
    assert_eq!(in_h1[0].id.to_string(), h1.id, "March belongs to H1");

    // Boundary inclusive: the H1 end instant still belongs to H1.
    let at_end_h1 = repo.find_by_date(utc("2026-06-30T23:59:59Z")).await.unwrap();
    assert_eq!(at_end_h1.len(), 1);
    assert_eq!(at_end_h1[0].id.to_string(), h1.id);

    // The very next second is H2, not H1.
    let at_start_h2 = repo.find_by_date(utc("2026-07-01T00:00:00Z")).await.unwrap();
    assert_eq!(at_start_h2.len(), 1);
    assert_eq!(at_start_h2[0].id.to_string(), h2.id);

    // Inside H2 → H2.
    let in_h2 = repo.find_by_date(utc("2026-10-01T12:00:00Z")).await.unwrap();
    assert_eq!(in_h2.len(), 1);
    assert_eq!(in_h2[0].id.to_string(), h2.id);

    // A date before both periods resolves to nothing (legacy / pre-setup).
    let before = repo.find_by_date(utc("2025-11-30T00:00:00Z")).await.unwrap();
    assert!(before.is_empty(), "a date before any period has no current period");
}
...
async fn overlapping_period_is_rejected_and_not_persisted() {
    ...
    uc.execute(cmd("2026-01-01T00:00:00Z", "2026-12-31T23:59:59Z")).await.expect("create 2026");

    // Entirely inside the 2026 window → overlap.
    let err = uc.execute(cmd("2026-06-01T00:00:00Z", "2026-06-30T23:59:59Z")).await.unwrap_err();
    assert!(matches!(err, AppError::Conflict(_)), "overlap must be a Conflict, got {err:?}");

    // Spanning into the existing window → overlap.
    let err2 = uc.execute(cmd("2026-12-01T00:00:00Z", "2027-03-31T23:59:59Z")).await.unwrap_err();
    assert!(matches!(err2, AppError::Conflict(_)), "partial overlap must be a Conflict");

    // A period strictly after the existing window is fine.
    let ok = uc.execute(cmd("2027-01-01T00:00:00Z", "2027-12-31T23:59:59Z")).await;
    assert!(ok.is_ok(), "non-overlapping future period must be accepted");

    assert_eq!(repo.list().await.unwrap().len(), 2, "only the two valid periods are persisted");
}

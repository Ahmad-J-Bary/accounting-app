...
use application::use_cases::purchase_return::{
    CreatePurchaseReturnUseCase, PostPurchaseReturnUseCase, PurchaseReturnQueries,
};
use application::use_cases::sales_return::{
    CreateSalesReturnUseCase, PostSalesReturnUseCase, SalesReturnQueries,
};
use domain::shared::ids::{PurchaseReturnId, SalesReturnId};
        ...
        if payment.reference.as_deref() == Some(reference) {
            ...
            if let Some(ref entry_number) = payment.journal_entry_number {
                if let Some(entry) = state
                    .journal_entry_repo
                    .find_by_number(entry_number)
                    .await
                    .map_err(|e| format!("فشل البحث عن قيد اليومية: {}", e))?
                {
                    state
                        .journal_entry_repo
                        .delete(&entry.id)
...
async fn reverse_sales_return_impact(
    ...
    let entries = state
        .journal_entry_repo
        .find_all_by_source_id(&existing.id.0.to_string())
        .await
    ...
    let partner_settlement = entries
        .first()
        .and_then(|entry| {
            entry
                .lines
    ...
    state
        .stock_movement_repo
        .delete_by_reference(&existing.return_number, "SalesReturn")
        .await
    ...
    if partner_settlement > Decimal::ZERO || cash_amount > Decimal::ZERO {
        if let Some(customer) = state
            .customer_repo
            .find_by_id(&existing.customer_id)
            .await
            .map_err(|e| format!("فشل العثور على العميل: {}", e))?
        {
            if let Some(base_currency) = state
                .currency_repo
                .get_base_currency()
                .await
                .map_err(|e| format!("فشل العثور على العملة الأساسية: {}", e))?
            {
                ...
                if partner_settlement > Decimal::ZERO {
                    let converted =
                        application::use_cases::unified_invoice::post::convert_to_partner_currency(
                            partner_settlement,
                            &base_currency.code,
                        ...
                        .await
                        .map_err(|e| format!("فشل تحويل العملة: {}", e))?;
                    updated_customer
                        .increase_debit(converted)
...
async fn reverse_purchase_return_impact(
    ...
    state
        .stock_movement_repo
        .delete_by_reference(&existing.return_number, "PurchaseReturn")
        .await
        .map_err(|e| format!("فشل حذف حركات المخزون: {}", e))?;
    ...
    if partner_settlement > Decimal::ZERO || cash_amount > Decimal::ZERO {
        if let Some(supplier) = state
            .supplier_repo
            .find_by_id(&existing.supplier_id)
            .await
...
#[tauri::command]
pub async fn delete_sales_return(state: State<'_, AppState>, id: String) -> Result<(), String> {
    let rid = SalesReturnId::from_str(&id).map_err(|_| "معرف المرتجع غير صالح".to_string())?;
...
#[tauri::command]
pub async fn delete_purchase_return(state: State<'_, AppState>, id: String) -> Result<(), String> {
    let rid = PurchaseReturnId::from_str(&id).map_err(|_| "معرف المرتجع غير صالح".to_string())?;
...
pub async fn create_sales_return(
    ...
    if let Some(ref edit_id) = request.id {
        let rid =
            SalesReturnId::from_str(edit_id).map_err(|_| "معرف المرتجع غير صالح".to_string())?;
        if let Some(existing) = state
            .sales_return_repo
            .find_by_id(&rid)
            .await
    ...
    let dto =
        CreateSalesReturnUseCase::new(state.sales_return_repo.clone(), state.customer_repo.clone())
            .execute(request)
            .await
            .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn list_sales_returns(state: State<'_, AppState>) -> Result<Vec<SalesReturnDto>, String> {
...
pub async fn create_purchase_return(
    ...
    if let Some(ref edit_id) = request.id {
        let rid =
            PurchaseReturnId::from_str(edit_id).map_err(|_| "معرف المرتجع غير صالح".to_string())?;
        if let Some(existing) = state
            .purchase_return_repo
            .find_by_id(&rid)
            .await

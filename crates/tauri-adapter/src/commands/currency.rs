...
use application::dto::currency_dto::{
    CreateCurrencyDto, CurrencyContextDto, CurrencyDto, ExchangeRateDto, SetExchangeRateDto,
    TodayRateStatusDto, UpdateCurrencyDto,
};
...
#[tauri::command]
pub async fn list_currencies(state: State<'_, AppState>) -> Result<Vec<CurrencyDto>, String> {
    state
        .currency_queries
        .list_all()
        .await
...
#[tauri::command]
pub async fn delete_currency(code: String, state: State<'_, AppState>) -> Result<(), String> {
    state
        .currency_commands
...
pub async fn list_rate_history(
    ...
    state
        .currency_queries
        .list_rate_history(&from, &to, limit)
        .await
        .map_err(|e: AppError| e.to_string())
}
#[tauri::command]
...
pub async fn setup_currencies(
    ...
    state
        .currency_setup
        .setup_currencies(&base_code, secondary_code.as_deref())
        .await
        .map_err(|e: AppError| e.to_string())
}

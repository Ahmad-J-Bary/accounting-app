...
use crate::dto::purchase_invoice_dto::{
    CreatePurchaseInvoiceRequest, PurchaseAdditionalCostDto, PurchaseInvoiceDto,
...
impl CreatePurchaseInvoiceUseCase {
    ...
    pub async fn execute(
    ...
    ) -> Result<PurchaseInvoiceDto, AppError> {
        let supplier_id: SupplierId = req
            .supplier_id
            .parse()
        ...
        let due_date = req
            .due_date
            .map(|d| DateTime::parse_from_rfc3339(&d).map(|dt| dt.with_timezone(&chrono::Utc)))
            .transpose()
            .map_err(|_| AppError::Invalid("تاريخ الاستحقاق غير صالح".into()))?;
        ...
        for item_req in req.items {
            let material_id: MaterialId = item_req
                .product_id
                .parse()
            ...
            let conversion_factor = item_req
                .conversion_factor
                .as_ref()
        ...
        if let Some(costs_req) = req.additional_costs {
            for cost_req in costs_req {
                let account_id: AccountId = cost_req
                    .account_id
                    .parse()
                    .map_err(|_| AppError::Invalid("معرف الحساب غير صالح".into()))?;
                let amount =
                    crate::utils::parse_decimal(Some(&cost_req.amount), "مبلغ التكلفة الإضافية")?;
...
impl ListPurchaseInvoicesUseCase {
    ...
    ) -> Result<Vec<PurchaseInvoiceDto>, AppError> {
        let invoices = if let Some(sid) = supplier_id {
            let supplier_id: SupplierId = sid
                .parse()
                .map_err(|_| AppError::Invalid("معرف المورد غير صالح".into()))?;
    ...
    pub async fn execute(&self, invoice_id: String) -> Result<PurchaseInvoiceDto, AppError> {
        ...
        for item in &invoice.items {
            if let Ok(Some(material)) = self.material_repo.find_by_id(&item.material_id).await {
                ...
                let effective_quantity =
                    item.quantity * item.conversion_factor.unwrap_or(rust_decimal::Decimal::ONE);
        ...
        for cost in invoice.additional_costs.iter() {
            ...
            let mut cost_entry = JournalEntry::create_purchase_costs_entry(
                ...
                format!(
                    "{} - فاتورة رقم {}",
                    cost.description, invoice.invoice_number
                ),
                invoice.invoice_date,
        ...
        self.repo
            .post_with_accounting(&invoice, &movements, &entries)
            .await?;
        Ok(enrich_invoice(
            invoice,
            &self.supplier_repo,
...
impl GetPurchaseInvoiceUseCase {
    ...
    pub async fn execute(&self, id: &str) -> Result<PurchaseInvoiceDto, AppError> {
        let id: PurchaseInvoiceId = id
            .parse()
            .map_err(|_| AppError::Invalid("معرف غير صالح".into()))?;
        let inv = self
            .repo
            .find_by_id(&id)
            .await?

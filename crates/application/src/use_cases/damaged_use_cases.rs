...
impl CreateDamagedItemUseCase {
    pub fn new(
        ...
        Self { repo, product_repo, movement_repo, journal_repo, account_repo }
    }
    ...
    pub async fn execute(&self, req: CreateDamagedItemRequest) -> Result<DamagedItemDto, AppError> {
        ...
        let item = DamagedItem::new(product_id.clone(), quantity, req.reason.clone(), damage_date, cost_impact, req.notes.clone())
        ...
        if cost_impact > Decimal::ZERO {
            let loss_account = self.account_repo.find_by_code("5105").await?
                .ok_or_else(|| AppError::NotFound("حساب خسائر المواد التالفة (5105) غير موجود".into()))?;
...
impl ListDamagedItemsUseCase {
    pub fn new(repo: Arc<dyn DamagedItemRepository>, product_repo: Arc<dyn ProductRepository>) -> Self {

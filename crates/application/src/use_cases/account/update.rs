...
impl UpdateAccountUseCase {
    ...
    pub async fn execute(
        ...
        AccountValidation::ensure_code_not_exists(&*self.account_repo, &cmd.code, Some(&id))
            .await?;
        ...
        account.linked_customer_id = cmd
            .linked_customer_id
            .as_deref()
            .and_then(|s| s.parse::<CustomerId>().ok());
        account.linked_supplier_id = cmd
            .linked_supplier_id
            .as_deref()
        ...
        account.debit = cmd
            ...
            .unwrap_or(account.debit);
        account.credit = cmd
            .credit
            .as_deref()
            .and_then(|s| Decimal::from_str(s).ok())
            .unwrap_or(account.credit);
            ...
            && account.linked_supplier_id.is_none()
        {
            let opening_window = opening_window_active(&self.opening_migration_repo)
                .await
                .map_err(|e| AccountUseCaseError::RepositoryError(e.to_string()))?;
            let is_receivable_or_payable = account
                .parent_id
                .as_ref()
                .map(|p| {
                    let pid = p.to_string();
        ...
        if account.code != old_code {
            self.update_children_codes(&account, &old_code, &account.code)
                .await?;
        }
            ...
            if let Some(ref customer_repo) = self.customer_repo {
                if let Ok(Some(mut customer)) = customer_repo.find_by_id(customer_id).await {
                    let debit = cmd
                        ...
                        .as_deref()
                        .and_then(|s| Decimal::from_str(s).ok())
                        .unwrap_or(customer.debit);
                    let credit = cmd
                        .credit
                        .as_deref()
                        .and_then(|s| Decimal::from_str(s).ok())
                        .unwrap_or(customer.credit);
                    let currency = cmd
                        .currency
                        .as_deref()
            ...
            if let Some(ref supplier_repo) = self.supplier_repo {
                if let Ok(Some(mut supplier)) = supplier_repo.find_by_id(supplier_id).await {
                    let debit = cmd
                        ...
                        .as_deref()
                        .and_then(|s| Decimal::from_str(s).ok())
                        .unwrap_or(supplier.debit);
                    let credit = cmd
                        .credit
                        .as_deref()
                        .and_then(|s| Decimal::from_str(s).ok())
                        .unwrap_or(supplier.credit);
                    let currency = cmd
                        .currency
                        .as_deref()
                        .map(|s| Currency::new(s, s, s, "", 2, false))
    ...
    async fn update_children_codes(
        &self,
        parent: &Account,
        old_parent_code: &str,
        new_parent_code: &str,
        ...
        let account_map: std::collections::HashMap<String, Account> = all_accounts
            .into_iter()

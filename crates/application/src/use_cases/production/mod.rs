use crate::dto::production_dto::{
    CreateProductionOrderRequest, ProductionMaterialDto, ProductionOrderDto, ProductionOutputDto,
};
...
impl CreateProductionOrderUseCase {
    ...
    ) -> Result<ProductionOrderDto, AppError> {
...
impl GetProductionOrderUseCase {
    ...
    pub async fn execute(&self, id: String) -> Result<ProductionOrderDto, AppError> {
        let oid = id
            .parse()
            .map_err(|_| AppError::NotFound("معرف الأمر غير صالح".into()))?;
        let order = self
            .repo
            .find_by_id(&oid)
            .await?

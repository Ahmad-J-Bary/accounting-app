...
impl PostPurchaseReturnUseCase {
    ...
    pub async fn execute(&self, id: String, settlement_mode: Option<String>, settlement_amount: Option<String>, is_paid: Option<bool>) -> Result<PurchaseReturnDto, AppError> {
        ...
        for line in &ret.lines {
            ...
            let movement_notes = if custom_notes.is_empty() {
                format!("مرتجع مشتريات رقم {} - رقم الفاتورة {}", ret.return_number, ret.return_number)
            } else {
        ...
        if cash_actually_paid {
            ...
            cash_journal_lines.push(JournalLine::new(
                ...
                MonetaryAmount::zero(doc_currency.clone()),
                format!("سند قبض من مورد مرتبط بمرتجع مشتريات رقم {}", ret.return_number),
            ));
        ...
        if let Some(supplier) = self.supplier_repo.find_by_id(&ret.supplier_id).await? {
            ...
            if cash_actually_paid {
                ...
                updated_supplier.increase_credit(converted_cash)
                    .map_err(|e| AppError::Invalid(e.to_string()))?;
            }
    ...
    async fn compute_settlement(
        ...
        let partner_balance = if let Some(supplier) = self.supplier_repo.find_by_id(supplier_id).await? {
            supplier.credit
        } else {
            Decimal::ZERO
        };

...
use domain::shared::ids::{CustomerId, SalesReturnId};
...
impl PostSalesReturnUseCase {
    ...
    pub async fn execute(
        ...
        is_paid: Option<bool>,
    ) -> Result<SalesReturnDto, AppError> {
        ...
        if cash_actually_paid {
                ...
                if let Some(cust_acc_id) = customer.account_id {
                    cash_journal_lines.push(
                        JournalLine::new(
                            ...
                            MonetaryAmount::zero(doc_currency.clone()),
                            format!(
                                "سند دفع لعميل مرتبط بمرتجع مبيعات رقم {}",
                                ret.return_number
                            ),
                        )
                        .with_partner(ret.customer_id.0),
            ...
            let mut cash_entry = JournalEntry::new(
                ...
                Utc::now(),
                format!(
                    "سند دفع لعميل مرتبط بمرتجع المبيعات رقم {}",
                    ret.return_number
                ),
                Some(ret.id.0.to_string()),
            )
            ...
            if let Some(customer) = self.customer_repo.find_by_id(&ret.customer_id).await? {
                if let Some(cust_acc_id) = customer.account_id {
                    let mut payment = Payment::new(
                        ...
                        Some(format!(
                            "دفعة نقدية مرتبطة بمرتجع مبيعات {}",
                            ret.return_number
                        )),
                    )
                    .map_err(|e| AppError::Invalid(e.to_string()))?;
        ...
        let mut customer_mutations = Vec::new();
        if let Some(customer) = self.customer_repo.find_by_id(&ret.customer_id).await? {
            let converted_total =
            ...
            updated_customer
                .decrease_debit(converted_total)
                .map_err(|e| AppError::Invalid(e.to_string()))?;
            ...
            if cash_actually_paid {
                let converted_cash =
                    ...
                    .await?;
                updated_customer
                    .increase_debit(converted_cash)
                    .map_err(|e| AppError::Invalid(e.to_string()))?;
            }

            customer_mutations.push(updated_customer);
        }
    ...
    async fn compute_settlement(
        ...
        let partner_balance =
            if let Some(customer) = self.customer_repo.find_by_id(customer_id).await? {
                customer.debit
            } else {
                Decimal::ZERO
            };

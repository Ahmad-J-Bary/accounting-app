    ...
    pub async fn execute(&self, req: CreateMaterialRequest) -> Result<MaterialDto, AppError> {
        ...
        if category_ids.is_empty() {
            if let Some(cat) = self
                .category_repo
                .find_by_name(DEFAULT_CATEGORY_NAME)
                .await?
            {
        ...
        for u in req.units {
            let factor = u.conversion_factor.parse().map_err(|_| {
                AppError::Invalid(format!("معامل تعبئة غير صالح للوحدة {}", u.name))
            })?;
        ...
        material.default_purchase_unit_id =
            resolve_default_unit_id(req.default_purchase_unit_id, &material.units)?;
        material.default_sale_unit_id =
            resolve_default_unit_id(req.default_sale_unit_id, &material.units)?;

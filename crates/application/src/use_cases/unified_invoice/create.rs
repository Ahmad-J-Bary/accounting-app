...
impl CreateInvoiceUseCase {
    ...
    pub async fn execute(&self, req: CreateInvoiceRequest) -> Result<InvoiceDto, AppError> {
        ...
        let amount_paid = Money::new(
            Decimal::from_str(&req.amount_paid).unwrap_or(Decimal::ZERO),
        ...
        let invoice_number = self
            .repo
            .get_next_invoice_number(invoice_type.clone())
            .await?;
        ...
        for line_dto in req.lines {
            ...
            let unit_price_original = line_dto.unit_price_original.and_then(|s| {
                Decimal::from_str(&s)
                    .ok()
                    .map(|amt| Money::new(amt, doc_currency.clone()))
            });
            let purchase_price_original = line_dto.purchase_price_original.and_then(|s| {
                Decimal::from_str(&s)
                    .ok()
                    .map(|amt| Money::new(amt, doc_currency.clone()))
            });
            let profit_amount_original = line_dto.profit_amount_original.and_then(|s| {
                Decimal::from_str(&s)
                    .ok()

use crate::dto::currency_dto::{CurrencyContextDto, CurrencyDto};
    ...
    pub async fn setup_currencies(
        ...
        if code.len() < 3 {
            return Err(AppError::Invalid(
                "رمز العملة الأساسية يجب أن يكون 3 أحرف على الأقل".to_string(),
            ));
        }

        let all = world_currencies::get_world_currencies();
        let base_info = all.iter().find(|c| c.code == code).ok_or_else(|| {
            AppError::Invalid(format!("العملة {} غير موجودة في القائمة العالمية", code))
        })?;
        ...
        for c in existing {
            if c.code != code
                && secondary_code
                    .as_ref()
                    .is_none_or(|s| c.code != s.trim().to_uppercase())
            {

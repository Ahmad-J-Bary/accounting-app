...
impl CreateInvoiceUseCase {
    ...
    pub async fn execute(&self, request: CreateInvoiceRequest) -> Result<InvoiceDto, AppError> {
        let customer_id_str = request
            .customer_id
            .ok_or_else(|| AppError::Invalid("معرف العميل مطلوب".into()))?;
        ...
        let discount_amount =
            crate::utils::parse_decimal(Some(&request.discount_amount), "قيمة الخصم")?;

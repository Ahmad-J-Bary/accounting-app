...
const { useCreate, useDelete } = createEntityMutations<CreatePaymentRequest, unknown>({
  ...
  mutations: {
    create: { fn: (req) => paymentService.createPayment(req), successMsg: "تم تسجيل السند بنجاح", errorMsg: "فشل تسجيل السند", extraInvalidations: ALL_REPORT_KEYS },
    delete: { fn: (id)  => paymentService.deletePayment(id),  successMsg: "تم حذف السند بنجاح",    errorMsg: "فشل حذف السند",    extraInvalidations: ALL_REPORT_KEYS },
  },
});

...
export function GenericDocumentGrid({
  ...
  const [searchType, setSearchType] = useState<"name" | "code" | "barcode">(
    "name",
  );
  ...
  const findNextCol = useCallback(
    (fromIdx: number, dir: 1 | -1): number => {
      ...
      if (fromIdx >= editableCols.length)
        idx = dir === 1 ? 0 : editableCols.length - 1;
      ...
      switch (e.key) {
        ...
        case "Enter": {
          ...
          if (nc >= editableCols.length) {
            ...
            nr = rowIdx + 1;
            const hasMaterial =
              lines[rowIdx]?.material_id || lines[rowIdx]?.material_name;
        ...
        case "ArrowDown":
          ...
          setSearchRow(null);
          inputRefs.current
            .get(`${Math.min(rowIdx + 1, lines.length - 1)}-${colIdx}`)
            ?.focus();
          break;
        case "ArrowUp":
          e.preventDefault();
          setSearchRow(null);
          inputRefs.current
            .get(`${Math.max(rowIdx - 1, 0)}-${colIdx}`)
            ?.focus();
          break;
        case "Delete":
          if (e.ctrlKey) {
  ...
  const formatRawAmount = useCallback(
    (amount: number, currencyCode?: string) => {
      const currency =
        currencies.find((c) => c.code === currencyCode) || baseCurrency || null;
      const formatted = formatWithLocale(amount, currency?.decimals ?? 2);
      return currency
        ? `${formatted} ${currency.symbol || currency.code}`
        : formatted;
    },
  ...
  const handleCellChange = useCallback(
    (rowIdx: number, colKey: string, value: string) => {
      if (readOnly) return;
      // Detect currency-specific fields: field_CCC (for example retail_price_EUR)
          ...
          return (
            ...
            >
              ...
              {filteredColumns.map((col) => {
                ...
                const isCellActive =
                  activeCell?.row === rowIdx && activeCell?.col === editColIdx;
                    ...
                    >
                      {src ? (
                        <img
                          src={src}
                          alt=""
                          className="w-6 h-6 object-contain rounded bg-muted border border-border"
                        />
                      ) : (
                  ...
                  return (
                    ...
                    >
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent
                          align="center"
                          className="min-w-[100px] shadow-xl"
                        >
                          <DropdownMenuLabel className="text-right text-[9px] font-black text-muted-foreground uppercase">
                            الوحدات المتاحة
                          </DropdownMenuLabel>
                          <DropdownMenuSeparator />
                          {units.map((u) => (
                            <DropdownMenuCheckboxItem
                              key={u.id}
                              checked={
                                line.unit_id === u.id || currentUnit === u.name
                              }
                              onCheckedChange={() => {
                                onUpdateLine(rowIdx, {
                                  unit_id: u.id,
                ...
                if (
                  col.type === "material" ||
                  col.type === "material_code" ||
                  col.type === "material_barcode"
                ) {
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-muted/80 border-t border-slate-200 text-[10px] text-muted-foreground font-bold">
        <div className="flex items-center gap-4 uppercase tracking-widest">
          <div className="flex items-center gap-1">
            <kbd className="bg-slate-200 px-1 rounded text-slate-600">
              Tab/Enter
            </kbd>{" "}
            انتقال
          </div>
          <div className="flex items-center gap-1">
            <kbd className="bg-slate-200 px-1 rounded text-slate-600">↑↓</kbd>{" "}
            صفوف
          </div>
          <div className="flex items-center gap-1">
            <kbd className="bg-slate-200 px-1 rounded text-slate-600">
        ...
        <div className="text-slate-500">إجمالي عدد الأسطر: {lines.length}</div>
      </div>
    </div>
  );
}

...
export function SidebarActionBar({ actions, className, maxColumns = 2 }: SidebarActionBarProps) {
  const visible = actions.filter(a => !a.hidden);
  if (visible.length === 0) return null;

  const cols = visible.length === 1 ? 1 : Math.min(visible.length, maxColumns);
  ...
  return (
    ...
    >
      {visible.map((act, idx) => (
        ...
        >
          <span className={cn("shrink-0 w-4 h-4 flex items-center justify-center", ACTION_ICON_COLORS[act.variant ?? 'secondary'])}>
            {act.icon}
          </span>
          {act.label}
        </button>

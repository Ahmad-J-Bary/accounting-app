      ...
      <div className="space-y-6">
        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-black text-slate-800">{t("about.appName", { namespace: "settings",  })}</h3>
              <p className="text-sm text-slate-500">{t("about.appTagline", { namespace: "settings",  })}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="space-y-1">
              <span className="text-slate-400 font-medium">{t("about.currentVersion", { namespace: "settings",  })}</span>
              <p className="font-black text-slate-800 font-mono" dir="ltr">{pkg.version}</p>
            </div>
            <div className="space-y-1">
              <span className="text-slate-400 font-medium">{t("about.latestVersion", { namespace: "settings",  })}</span>
          ...
          {phase === "ready" && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 space-y-3">
              <p className="text-sm font-bold text-emerald-700 text-center">{t("about.readyToInstall", { namespace: "settings",  })}</p>
              <Button
                size="sm"
          ...
          {updateInfo && !updateInfo.has_update && !isUpdating && updateInfo.latest_version !== "فشل الاتصال" && (
            <div className="bg-slate-100 rounded-xl p-4">
              <p className="text-sm font-bold text-slate-600">{t("about.upToDate", { namespace: "settings",  })}</p>
            </div>
          )}
        </div>
      </div>

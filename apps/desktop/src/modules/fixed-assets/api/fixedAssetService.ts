...
function toAssetPayload(req: CreateFixedAssetRequest) {
  return {
    code: req.code,
    name: req.name,
...
export const fixedAssetService = {
  ...
  async create(req: CreateFixedAssetRequest): Promise<string> {
    return await invoke<string>('create_fixed_asset', toAssetPayload(req));
  },

  async update(id: string, req: CreateFixedAssetRequest): Promise<void> {
    return await invoke<void>('update_fixed_asset', { id, ...toAssetPayload(req) });
  },

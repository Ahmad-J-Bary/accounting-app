...
export function JournalTable({ entries, loading, visibleColumns, filters }: JournalTableProps) {
  ...
  const allColumns = useMemo<Column<typeof tableData[0]>[]>(() => [
    ...
    { id: "total_debit_usd",  header: "عليه / مدين ($)",      accessor: (e) => e.debit_usd > 0 ? `${e.debit_usd.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})} $` : "",    align: "left", className: "tabular-nums font-black text-blue-700 text-[11px]" },
    { id: "total_debit_syp",  header: "عليه / مدين (ل.س)",    accessor: (e) => e.debit_syp > 0 ? `${e.debit_syp.toLocaleString('en-US',{maximumFractionDigits:0})} ل.س` : "",      align: "left", className: "tabular-nums font-black text-blue-700 text-[11px]" },
    { id: "total_credit_usd", header: "له / دائن ($)",        accessor: (e) => e.credit_usd > 0 ? `${e.credit_usd.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})} $` : "",  align: "left", className: "tabular-nums font-black text-emerald-700 text-[11px]" },
    { id: "total_credit_syp", header: "له / دائن (ل.س)",      accessor: (e) => e.credit_syp > 0 ? `${e.credit_syp.toLocaleString('en-US',{maximumFractionDigits:0})} ل.س` : "",    align: "left", className: "tabular-nums font-black text-emerald-700 text-[11px]" },

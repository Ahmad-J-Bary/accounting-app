...
export function CreatePeriodCard({
  ...
  return (
    <SectionCard title={t("fiscalPeriods.createCard.title", { namespace: "accounting",  })}>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <FieldLabel htmlFor="fp-start" required>{t("fiscalPeriods.createCard.start", { namespace: "accounting",  })}</FieldLabel>
          <Input id="fp-start" type="date" value={start} onChange={(e) => onStartChange(e.target.value)} className="h-9" aria-required />
        </div>
        <div className="space-y-1.5">
          <FieldLabel htmlFor="fp-end" required>{t("fiscalPeriods.createCard.end", { namespace: "accounting",  })}</FieldLabel>
      ...
      {start && end && !canCreate && (
        <p className="text-2xs text-red-600" role="alert">{t("fiscalPeriods.createCard.rangeError", { namespace: "accounting",  })}</p>
      )}
